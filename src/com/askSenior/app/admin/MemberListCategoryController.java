...
public class MemberListCategoryController implements Execute{
	...
	public Result execute(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServerException {
		...
		criteria.setType(req.getParameter("order"));
		
		AdminDAO adminDAO = new AdminDAO();

		
		adminDAO.categoryAll(criteria).stream().map(adminVO -> new JSONObject(adminVO)).forEach(admin -> admins.put(admin));
		
		out.write(admins.toString());
		out.close();
		
		 // resp.setAttribute("admins", admins.toString());
		return null;
	}
